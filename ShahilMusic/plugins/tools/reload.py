...
from ShahilMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from ShahilMusic.utils.decorators import ActualAdminCB, AdminActual, language
from ShahilMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Shahil.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Shahil.stop_stream_force(chat_id)
        except:
            pass
...
async def get_vars(_, message: Message):
    try:
        await app.send_message(
            ...
            text=f"""<u>**{app.name} ᴄᴏɴғɪɢ ᴠᴀʀɪᴀʙʟᴇs :**</u>
